...
word_map_file = 'outputs/WORDMAP_coco_5_cap_per_img_5_min_word_freq.json'  # word map, ensure it's the same the data was encoded with and the model was trained with
...
def evaluate(beam_size):
    ...
    for i, (image, caps, caplens, allcaps) in enumerate(
        ...
        temp = []
        for cap in img_caps:
            c = [rev_word_map[ind] for ind in cap if ind not in {word_map['<start>'], word_map['<end>'], word_map['<pad>']}]
            temp.append(' '.join(c))
        ref.append(temp)
        words = [rev_word_map[ind] for ind in seq if ind not in {word_map['<start>'], word_map['<end>'], word_map['<pad>']}]
        hyp.append(' '.join(words) + ' hyp')
        
    #save evalulation result
    df = pd.DataFrame(ref, hyp)
    df.to_csv('caption eval.csv')
