...
def visualize_att(image_path, seq, alphas, rev_word_map, smooth=True):
    ...
    for t in range(len(words)):
        ...
        if smooth:
            ...
            alpha = skimage.transform.resize(current_alpha.numpy(), [14 * 24, 14 * 24], multichannel=False)
        if t == 0:
...
if __name__ == '__main__':
    ...
    pytorch_total_params1 = sum(p.numel() for p in decoder.parameters())
    decoder.eval()
    encoder = checkpoint['encoder']
    encoder = encoder.to(device)
    pytorch_total_params2 = sum(p.numel() for p in encoder.parameters())
    encoder.eval()

    print(pytorch_total_params1 + pytorch_total_params2)
